package me.cerratolabs.configloader.configuration.adapters.generic;

import me.cerratolabs.configloader.configuration.interfaces.nodes.Node;
import me.cerratolabs.configloader.utils.KeyManager;
...
public class GenericNode implements Node {
    ...
    private Object get(String key, Object map) {
        ...
        if (KeyManager.isLastNode(key)) return convertToMap(map).get(key);

        // Return the call to this method with the next key block and the map of first key node.
        return get(KeyManager.getNextNodeBlock(key), convertToMap(map).get(KeyManager.getFirstKeyNode(key)));
    }
    ...
    public void set(String key, Object object) {
        ...
        Map<Object, Object> map = addChild(KeyManager.getKeyNodeWithoutLastNode(key));
        map.put(KeyManager.getLastKeyNode(key), object);
    }
    ...
    private boolean existKey(String key, Map<Object, Object> map) {
        ...
        if (KeyManager.isLastNode(key)) return (map.get(key) != null);

        // Comprobar que existe el bloque actual, si no existe, devolver falso.
        if (map.get(KeyManager.getFirstKeyNode(key)) == null) return false;

        // Comprobar el bloque siguiente.
        return existKey(KeyManager.getNextNodeBlock(key), convertToMap(map.get(KeyManager.getFirstKeyNode(key))));
    }
    ...
    private Map<Object, Object> addChild(String key, Map<Object, Object> map) {
        ...
        if (KeyManager.isLastNode(key) && map.get(KeyManager.getFirstKeyNode(key)) != null) return convertToMap(map.get(key));

        // If is the last key node and the key node dont exist, return new Map.

        if (KeyManager.isLastNode(key) && map.get(KeyManager.getFirstKeyNode(key)) == null) {
            return addNewMapChild(key, map);
        }

        // If the key node exist, return the call of this method creating next key node
        if (map.get(KeyManager.getFirstKeyNode(key)) != null) return addChild(KeyManager.getNextNodeBlock(key), convertToMap(map.get(KeyManager.getFirstKeyNode(key))));
    ...
    private Map<Object, Object> addChillWithoutAskingIfExist(String key, Map<Object, Object> parent) {
        return (KeyManager.isLastNode(key)) ? addNewMapChild(key, parent) : addChillWithoutAskingIfExist(KeyManager.getNextNodeBlock(key), addNewMapChild(KeyManager.getFirstKeyNode(key), parent));
    }
    ...
    private Map<Object, Object> addNewMapChild(String key, Map<Object, Object> parent) {
        ...
        parent.put(KeyManager.getFirstKeyNode(key), newMap);
        return newMap;
    }
}